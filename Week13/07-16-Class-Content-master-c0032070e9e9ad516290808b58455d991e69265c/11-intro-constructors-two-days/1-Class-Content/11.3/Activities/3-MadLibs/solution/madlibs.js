...
var cbs = {
    gotVerb: gotVerb,
    gotAdjective: gotAdjective,
    gotNoun: gotNoun
};

function gotNoun (answer) {
...
function getWords(loop, cbs){
	// We get two nouns.
	if(loop < 2){
		inquirer.prompt({ name:"noun", message:"Enter a noun: " }).then(cbs.gotNoun);
	}

	// Then we get an adjective.
	if(loop >= 2 && loop < 3){
		inquirer.prompt({ name:"adjective", message: "Enter an adjective: " }).then(cbs.gotAdjective);
	}

	// Then, a verb.
	if(loop >= 3 && loop < 4){
		inquirer.prompt({ name:"verb", message:"Enter a verb: " }).then(cbs.gotVerb);
	}
